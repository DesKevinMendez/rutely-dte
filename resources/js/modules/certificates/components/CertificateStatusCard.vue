...
import type { CertificateEnvironment, CertificateMetadata } from '../types/certificate.types';
...
<template>
    <Card title="Estado del Certificado" :subtitle="`Ambiente seleccionado: ${props.selectedEnvironment}`">
        <div v-if="props.metadata" class="space-y-5">
            ...
            <div class="rounded-xl border border-emerald-200 bg-emerald-50 p-3 text-xs text-emerald-800 dark:border-emerald-800/60 dark:bg-emerald-950/40 dark:text-emerald-300">
                Certificado verificado y encriptado con seguridad <strong>AES-256-GCM</strong> en la base de datos.
            </div>
        </div>
        ...
        <div v-else class="space-y-2 py-6 text-center">
            ...
            <p class="text-sm font-bold text-gray-900 dark:text-white">Sin Certificado Registrado</p>
            <p class="text-xs text-gray-500 dark:text-gray-400">No hay ningún certificado registrado para el ambiente de <strong>{{ props.selectedEnvironment }}</strong>.</p>
        </div>
    </Card>
</template>
