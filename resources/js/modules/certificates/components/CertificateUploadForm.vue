...
<template>
    <Card title="Cargar Certificado Digital (MH)" subtitle="Seleccione el archivo XML del certificado de firma entregado por el Ministerio de Hacienda.">
        <form class="space-y-5" @submit.prevent="submit">
            <div v-if="props.uploadSuccess" class="rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-800 dark:border-emerald-800 dark:bg-emerald-950/60 dark:text-emerald-300">
                Certificado cargado en la simulación UI para el ambiente de <strong>{{ props.selectedEnvironment }}</strong>.
            </div>
            ...
            <div>
                <label for="certificate-file" class="mb-2 block text-sm font-semibold text-gray-700 dark:text-gray-300">
                ...
                <label
                    ...
                    :class="isDragging ? 'border-primary-500 bg-primary-50 dark:bg-primary-950/20' : ''"
                    @dragenter.prevent="isDragging = true"
                    @dragover.prevent="isDragging = true"
                    @dragleave.prevent="isDragging = false"
                    @drop.prevent="handleDrop"
                >
                    <span class="mb-3 flex h-12 w-12 items-center justify-center rounded-full bg-primary-50 text-primary-500 dark:bg-primary-950/40 dark:text-primary-400">
                        <IconCloudUpload :size="24" :stroke="1.75" />
                    </span>
                    <span class="text-sm font-semibold text-gray-900 dark:text-white">
                        {{ fileName || 'Hacé clic para seleccionar o arrastrá el archivo' }}
                    </span>
                    <span class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                        Archivos soportados: .xml, .crt o .txt
                    </span>
                </label>
                <input id="certificate-file" type="file" class="sr-only" accept=".xml,.crt,.txt" @change="handleFileChange" />
            </div>
            ...
            />
            <p class="-mt-3 text-xs text-gray-500 dark:text-gray-400">La contraseña se valida contra el hash de la clave privada antes de guardar.</p>

            <div class="flex justify-end border-t border-gray-200 pt-3 dark:border-gray-700">
                <BaseButton type="submit" variant="primary" size="auto" :disabled="!fileName">Guardar y Encriptar Certificado</BaseButton>
            </div>
        </form>
    </Card>
</template>
