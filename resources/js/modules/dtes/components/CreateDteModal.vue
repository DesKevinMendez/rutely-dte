...
const items = ref<DteItem[]>([
    { descripcion: 'Servicios de Desarrollo de Software DTE', cantidad: 1, precioUni: 100 },
]);

const itemColumns: TableField<DteItem>[] = [
    { label: 'Descripción', key: 'descripcion', width: 320, slot: 'description' },
    { label: 'Cant.', key: 'cantidad', width: 120, slot: 'quantity' },
    { label: 'Precio Uni ($)', key: 'precioUni', width: 150, slot: 'unitPrice' },
];

const subtotal = computed(() => items.value.reduce((total, item) => total + Number(item.cantidad || 0) * Number(item.precioUni || 0), 0));
const iva = computed(() => tipoDte.value === '03' ? subtotal.value * 0.13 : 0);
...
const itemIndex = (row: DteItem): number => Math.max(items.value.indexOf(row), 0);
...
const reset = (): void => {
    ...
    items.value = [{ descripcion: 'Servicios de Desarrollo de Software DTE', cantidad: 1, precioUni: 100 }];
};
...
<template>
    ...
    >
        <form class="space-y-5 pt-2 text-sm" @submit.prevent="submit">
            <div class="rounded-xl border border-primary-200 bg-primary-50 p-3 text-xs text-primary-700 dark:border-primary-800 dark:bg-primary-950/40 dark:text-primary-300">
                Modo UI: esta emisión se agrega únicamente al listado local de demostración.
            </div>

            <FormSelect v-model="tipoDte" id="tipo-dte" name="tipoDte" label="Tipo de Comprobante" :options="typeOptions" />

            <Card title="Datos del Receptor / Cliente">
                <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
                    <div class="sm:col-span-2">
                        <FormInput v-model="receptorNombre" id="receptor-nombre" name="receptorNombre" type="text" label="Nombre o Razón Social" small />
                    </div>
                    <FormSelect v-model="tipoDocumento" id="tipo-documento" name="tipoDocumento" label="Tipo de Documento" small :options="documentOptions" />
                    <FormInput v-model="receptorDocumento" id="receptor-documento" name="receptorDocumento" type="text" label="Nº Documento" small />
                    <div class="sm:col-span-2">
                        <FormInput v-model="receptorCorreo" id="receptor-correo" name="receptorCorreo" type="email" label="Correo Electrónico" small />
                    </div>
                </div>
            </Card>
            ...
            <Card title="Ítems del Documento" col-in-mobile>
                <template #headerButtons>
                    <BaseButton type="button" variant="outline" size="small" @click="addItem">
                        Agregar Ítem
                    </BaseButton>
                </template>
            ...
            <Card>
                <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                    <span class="text-xs text-gray-500 dark:text-gray-400">El monto se calcula automáticamente según el tipo de comprobante.</span>
                    <div class="text-right text-xs text-gray-500 dark:text-gray-400">
                        <div>Subtotal: <strong>${{ subtotal.toFixed(2) }}</strong></div>
                        <div v-if="tipoDte === '03'">IVA (13%): <strong>${{ iva.toFixed(2) }}</strong></div>
                        <div class="mt-1 text-base font-extrabold text-primary-600 dark:text-primary-400">Total: ${{ total.toFixed(2) }} USD</div>
                    </div>
                </div>
            </Card>

            <div class="flex justify-end gap-2 border-t border-gray-200 pt-4 dark:border-gray-800">
                <BaseButton type="button" variant="outline" size="auto" @click="emit('update:isOpen', false)">Cancelar</BaseButton>
                <BaseButton type="submit" variant="primary" size="auto">Emitir DTE</BaseButton>
            </div>
        </form>
    </Modal>
</template>
