...
final class FirmadorService
{
    ...
    public function signDocument(
        ...
        if ($storedCertificate === null) {
            throw new DteSigningException('No hay material de firma configurado para la empresa y el ambiente seleccionados.');
        }

        if (! $storedCertificate->active) {
            throw new DteSigningException('El certificado configurado está inactivo.');
        }
        ...
        if ($certificateXml === '' || $password === '') {
            throw new DteSigningException('El material de firma configurado no es válido.');
        }
    ...
    private function assertCertificateIssuer(MhCertificate $certificate, string $issuerNit): void
    {
        if (
            ...
            throw new DteSigningException('El NIT del certificado no coincide con el NIT de la empresa.');
        }
    }
